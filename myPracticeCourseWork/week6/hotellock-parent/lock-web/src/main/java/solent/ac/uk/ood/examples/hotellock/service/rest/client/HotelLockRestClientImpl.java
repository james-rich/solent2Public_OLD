...
public class HotelLockRestClientImpl {
    ...
    /**
     * Client to use rest interface to unlock door
     * @param roomNumber
     * @param cardCode
     * @return 
     */
    public Boolean unlockDoor(String roomNumber, String cardCode) {

        Response response = null;
        try {
            Invocation.Builder builder = target.path("unlockDoor/"+roomNumber)
                    .queryParam("cardCode", cardCode)
                    .request(mediaType);
            response = builder.get();
        } catch (Exception e) {
            throw new RuntimeException("cannot run rest client to unlock the door: Exception:", e);
        }
